    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative py-20 text-white overflow-hidden">
        <div className="absolute inset-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-rose-600/90 via-pink-600/85 to-rose-600/90" />
        </div>
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          <div className="inline-flex items-center space-x-2 bg-white/10 backdrop-blur-md rounded-full px-4 py-2 mb-6 border border-white/20">
            ...
            <span className="text-sm font-medium text-white/90">News & Events</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-bold mb-6">
            Activities & <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-200 to-pink-200">Events</span>
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 mb-10 p-1.5 bg-white shadow-lg rounded-2xl h-14">
            <TabsTrigger value="upcoming" className="rounded-xl text-base font-medium data-[state=active]:bg-rose-500 data-[state=active]:text-white">
    ...
    <Link href={`/news/${activity.id}`}>
      <Card className={`overflow-hidden hover:shadow-xl transition-all duration-300 group cursor-pointer h-full ${isPast ? 'opacity-90' : ''}`}>
        ...
        <CardFooter className="px-5 pb-5 pt-0">
          <Button variant="ghost" className="w-full group-hover:bg-rose-50 group-hover:text-rose-600">
